...
T mapValue(T2 val, T2 in_min, T2 in_max, T out_min, T out_max) {
...
double fractionalHour(const tm& timeinfo) {
  ...
  return static_cast<double>(timeinfo.tm_hour) + mapValue<double>(totalSeconds, 0, 3600, 0.0, 1.0);
}
...
MoonPhaseResult computeMoonPhase(const DateTime& dt) {
  ...
  const double j = julianDay(t.tm_year + 1900, t.tm_mon + 1, static_cast<double>(t.tm_mday) + hour / 24.0) - 2444238.5;
