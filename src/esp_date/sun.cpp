...
double hourAngleSunrise(double lat, double solarDec) {
  ...
  double haArg = (std::cos(degToRad(kSunAngle)) / (std::cos(latRad) * std::cos(sdRad)) - std::tan(latRad) * std::tan(sdRad));
...
int sunriseSetLocalMinutes(bool isRise, int year, int month, int day, double latitude, double longitude, double offsetMinutes) {
...
SunCycleResult buildSunCycleResult(int minutes,
...
SunCycleResult ESPDate::sunrise(float latitude, float longitude, float timezoneHours, bool isDst) const {
  return sunrise(latitude, longitude, timezoneHours, isDst, now());
}

SunCycleResult ESPDate::sunset(float latitude, float longitude, float timezoneHours, bool isDst) const {
  return sunset(latitude, longitude, timezoneHours, isDst, now());
}

SunCycleResult ESPDate::sunrise(float latitude,
                                float longitude,
                                float timezoneHours,
                                bool isDst,
  ...
  const int minutes = sunriseSetLocalMinutes(true, localDate.year, localDate.month, localDate.day, latitude, longitude, offsetMinutes);
  return buildSunCycleResult(minutes, offsetMinutes, localDate, *this);
}

SunCycleResult ESPDate::sunset(float latitude,
                               float longitude,
                               float timezoneHours,
                               bool isDst,
                               const DateTime& day) const {
  ...
  const int minutes = sunriseSetLocalMinutes(false, localDate.year, localDate.month, localDate.day, latitude, longitude, offsetMinutes);
  return buildSunCycleResult(minutes, offsetMinutes, localDate, *this);
}
...
SunCycleResult ESPDate::sunrise(float latitude, float longitude, const char* timeZone, const DateTime& day) const {
  if (!validCoordinates(latitude, longitude)) {
    return SunCycleResult{false, DateTime{}};
  }
  ...
  const int minutes = sunriseSetLocalMinutes(true, data.date.year, data.date.month, data.date.day, latitude, longitude, data.offsetMinutes);
  return buildSunCycleResult(minutes, data.offsetMinutes, data.date, *this);
}

SunCycleResult ESPDate::sunset(float latitude, float longitude, const char* timeZone, const DateTime& day) const {
  if (!validCoordinates(latitude, longitude)) {
    return SunCycleResult{false, DateTime{}};
  }
  OffsetDateResult data = computeOffsetAndDate(day, timeZone);
  ...
  const int minutes = sunriseSetLocalMinutes(false, data.date.year, data.date.month, data.date.day, latitude, longitude, data.offsetMinutes);
  return buildSunCycleResult(minutes, data.offsetMinutes, data.date, *this);
}
...
SunCycleResult ESPDate::sunriseFromConfig(const DateTime& day) const {
  ...
  const int minutes = sunriseSetLocalMinutes(true, data.date.year, data.date.month, data.date.day, latitude_, longitude_, data.offsetMinutes);
  return buildSunCycleResult(minutes, data.offsetMinutes, data.date, *this);
}
...
SunCycleResult ESPDate::sunsetFromConfig(const DateTime& day) const {
  ...
  const int minutes = sunriseSetLocalMinutes(false, data.date.year, data.date.month, data.date.day, latitude_, longitude_, data.offsetMinutes);
  return buildSunCycleResult(minutes, data.offsetMinutes, data.date, *this);
}
...
bool ESPDate::isDayWithOffsets(const DateTime& day, int sunRiseOffsetSec, int sunSetOffsetSec) const {
